...
def get_all_threads(
    ...
    if not post:
        raise HTTPException(
            ...
            detail=f"Thread with id {vote.thread_id} does not exist",
        )
    vote_query = db.query(models.Vote).filter(
        models.Vote.thread_id == vote.thread_id, models.Vote.user_id == current_user.id
    )
    ...
    if vote.dir == 1:
        if found_vote:
            raise HTTPException(
                ...
                detail=f"user {current_user.id} has already voted on thread{vote.thread_id}",
            )
